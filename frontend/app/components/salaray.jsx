// app/dashboard/salary/[employeeId]/page.jsx
"use client";

import { useMemo, useState } from "react";
import Link from "next/link";

// ---- DEMO store (replace with real fetch later)
const DEMO = [
  {
    _id: "1",
    employeeId: "EMP-001",
...
const PKR = (n) =>
  (Number.isFinite(n) ? n : Number(n || 0)).toLocaleString("en-PK", {
    style: "currency",
    currency: "PKR",
    minimumFractionDigits: 0,
  });

export default function EmployeeSalaryPage({ params }) {
  const { employeeId } = params;

  // find demo employee by route param
  const emp = useMemo(
    () => DEMO.find((e) => e.employeeId === employeeId) || DEMO[0],
    [employeeId]
  );
  ...
  const onSave = () => {
    ...
    // later: POST { employeeId, month, salary: emp.salary, deduction, bonus, comments, payable }
  };
    ...
    <section className="rounded-2xl border bg-white p-4 md:p-6 shadow-sm">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-base md:text-lg font-semibold">Salary & Payable — {employeeId}</h3>
        </div>
        <button
          type="button"
          onClick={onSave}
          className="px-4 py-2 rounded-lg bg-gray-900 text-white hover:bg-black transition"
          title="Wire to backend later"
        >
          {saved ? "Saved ✓" : "Save"}
        </button>
      </div>

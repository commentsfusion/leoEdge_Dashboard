...
export const deleteUserByEmail = async (req, res) => {
  try {
    ...
    // only do self-protection / role checks if we actually have a requester
    if (requester) {
      ...
      if (requester.role === "moderator" && target.role !== "user") {
        return res.status(403).json({
          success: false,
          message: "Moderators can delete only user accounts",
        });
      }
    }
    ...
    return res.json({
      success: true,
      message: `Deleted ${target.email} successfully`,
    });
  } catch (err) {
